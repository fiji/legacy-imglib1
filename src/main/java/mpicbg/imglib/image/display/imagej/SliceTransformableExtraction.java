...
public abstract class SliceTransformableExtraction<T extends Type<T>> extends Thread implements Stopable
{
    ...
    public void run()
    {
    	...
    	final double[] initialPosition = new double[ numDimensions ];
		...
		final double[] position1 = initialPosition.clone();
		final double[] position2 = initialPosition.clone();
		...
		try
		{
	    	...
	    	if ( isAffine )
	    	{
	    		...
	    		final double[] vectorX = new double[ initialPosition.length ];
	    		final double[] vectorY = new double[ initialPosition.length ];
	    		...
	    		if ( sizeX > 1 )
	    		{
		    		...
		    		for ( int d = 0; d < initialPosition.length; ++d )
		    		{
		    			vectorX[ d ] = ( position2[ d ] - position1[ d ] ) / ( double )sizeX;
	    		...
	    		if ( sizeY > 1 )
	    		{
		    		...
		    		for ( int d = 0; d < initialPosition.length; ++d )
		    		{
		    			vectorY[ d ] = ( position2[ d ] - position1[ d ] ) / ( double )sizeY;
		    			position1[ d ] = initialPosition[ d ];
		    			position2[ d ] = initialPosition[ d ];
		    		}
	    		}
