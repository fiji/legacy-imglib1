...
public class ImageJVirtualDisplay<T extends Type<T>> extends ImageStack
{
			                     ...
			                     final int type, final int[] dim, final int[] dimensionPositions )
	{
		...
		for ( final InverseTransformDescription<T> it : interpolators )
		{
	...
	public void setParent( final ImagePlus parent ) { this.parent = parent; }
    ...
    public float[] extractSliceFloat( final int slice )
    {
		...
		for  ( final SliceTransformableExtraction<T> thread : threadList )
			thread.stopThread();

    	threadList.clear();

        for ( final InverseTransformDescription<T> it : transformDescription )
        {
        	final SliceTransformableExtractionFloat<T> thread = new SliceTransformableExtractionFloat<T>( transformDescription.size(), it, sliceImg, parent, dimensionPositions, dimX, dimY, dimZ, sizeX, sizeY, slice);
    ...
    public float[] extractSliceFloatSingleThreaded( final int slice )
    {
    	...
    	final double[] initialPosition = new double[ dimensionPositions.length ];
    	...
    	final double[] position = initialPosition.clone();
    ...
    @Override
    public String getSliceLabel(final int n) { return "" + n; }
